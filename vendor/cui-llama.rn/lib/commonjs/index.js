...
const installJsi = async () => {
  ...
  if (typeof global.llamaInitContext !== 'function') {
    // Bridgeless New Architecture (RN 0.83+) does not set global.__turboModuleProxy
    // when NativeRNLlama is first imported, so its cached default export can be null.
    // Resolve the module lazily here with a NativeModules fallback.
    const llamaModule = _NativeRNLlama.default ?? _reactNative.TurboModuleRegistry.get('RNLlama') ?? _reactNative.NativeModules.RNLlama ?? null;
    if (llamaModule == null) {
