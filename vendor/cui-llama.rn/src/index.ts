...
import RNLlama, { type Spec as RNLlamaSpec } from './NativeRNLlama'
import './jsi'
import type {
  NativeContextParams,
  NativeLlamaContext,
...
export const installJsi = async () => {
  ...
  if (typeof global.llamaInitContext !== 'function') {
    // The default export of ./NativeRNLlama resolves the TurboModule at import time
    // via TurboModuleRegistry.get('RNLlama'). Under the New Architecture in bridgeless
    // mode (RN 0.83+), global.__turboModuleProxy is not set when this module is first
    // imported, so that call returns null and the cached export stays null forever.
    // Resolve the module lazily here (when the runtime is fully initialized) and fall
    // back to NativeModules, which the bridgeless legacy-interop layer populates.
    const llamaModule: RNLlamaSpec | null =
      RNLlama ??
      TurboModuleRegistry.get<RNLlamaSpec>('RNLlama') ??
      ((NativeModules as Record<string, unknown>).RNLlama as RNLlamaSpec | undefined) ??
      null
    if (llamaModule == null) {
      throw new Error('JSI bindings not installed: RNLlama native module is unavailable')
    }
    const installed = await llamaModule.install()
